...
installer (on Windows) or just copy over `/Applications/Evoai-Qt` (on Mac)
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  EvoaiCoin), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in EvoaiCoin per 1,000
...
- #16521 Use the default maxfeerate value as EvoaiCoin/kB (Remagpie)
...
- #16929 follow-up to rpc: default maxfeerate value as EvoaiCoin/kB (jonatack)
