...
The data directory is the default location where the EvoaiCoin Core files are stored.

1. The default data directory paths for supported platforms are:

Platform | Data directory path
---------|--------------------
Linux    | `$HOME/.EvoaiCoin/`
macOS    | `$HOME/Library/Application Support/EvoaiCoin/`
Windows  | `%APPDATA%\EvoaiCoin\` <sup>[\[1\]](#note1)</sup>
...
`./`               | `bitcoin.conf`        | Contains [configuration settings](bitcoin-conf.md) for `evoaid` or `bitcoin-qt`; can be specified by `-conf` option
`./`               | `evoaid.pid`        | Stores the process ID (PID) of `evoaid` or `bitcoin-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `evoaid` or `bitcoin-qt`; can be specified by `-debuglogfile` option
